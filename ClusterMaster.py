...
class ClusterMaster:
    ...
    def initialize_kvalue(self):
        ...
        for i in range (self.numberOfCluster):
            self.kvalue[i] = [self.x[i], self.y[i], self.z[i]]
    ...
    def apply_minkowski(self):
        ...
        for i in tqdm(range (row_length)):
            print(f"\nInput x = {self.x[i]}\ty = {self.y[i]}\t z = {self.z[i]}")
            ...
            for j in range (self.numberOfCluster):
                ...
                distance[j] = self.minowski_equation(self.x[i], self.y[i], self.z[i], self.kvalue[j], self.p);
            ...
            self.kvalue[nearest_distance][2] = (self.kvalue[nearest_distance][2] + self.z[i]) / 2
            ...
            for k in range (self.numberOfCluster):
                print(f"Centroid{k+1}: x = {self.kvalue[k][0]} \t y = {self.kvalue[k][1]} \t z = {self.kvalue[k][2]}")
            time.sleep(0.1)

    #This method will use minkowski distance using actual formula, not the library and will return the distance
    def minowski_equation(self, x1,y1,z1, kvalue, p):
        distance = ((abs(kvalue[0] - x1) ** p) + (abs(kvalue[1] - y1) ** p) + (abs(kvalue[2] - z1) ** p)) ** (1 / p)
        return distance

    #This method will be use to retrieve the final output
    def get_result(self):
        self.dataframe = pd.DataFrame({self.column_names['first_column']: self.x, self.column_names['second_column']: self.y, self.column_names['third_column']: self.z, "Assignment": self.assignment})
